    ...
    <div className={`chat-conversation${right ? ' right' : ''}`} {...props}>
      <div className={`chat-message${previous ? ' previous' : ''}`}>
        ...
        { photos.length === 0
          ...
          : <div className="photos-container">{photos.length > 0 ? Array.from({ length: Math.min(photos.length, 4) }).fill(false).map((_, i) => <div key={`photoschat_${i}`} type="button" className="photo-gallery" alt="Sent photos" style={{backgroundImage: `url(${photos[i]})`}} onClick={() => handlePhotos(i)}></div>) : undefined}</div> }
      </div>
      { !previous ?
